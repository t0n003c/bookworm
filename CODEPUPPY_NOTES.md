...
| Trip Research mobile UX | ✅ Fixed (2026-05-13) | Add buttons shrink to `＋`; spot cards flip horizontal; resize listener |
...
| Date | What happened |
|---|---|
| 2026-05-13 | **Trip Research view — mobile UX fixes (commit `e748400`).** Three changes for small screens. **(1) `+ Add Location` / `+ Add Spot` buttons shrink to `＋` on mobile (< 640 px):** `_tripRenderTopbarControls()` in `home-page-trip.js` now checks `window.innerWidth < 640`; on narrow viewports both the button label and the search input width are condensed so the topbar row doesn’t overflow. `title` + `aria-label` attributes added so screen readers and long-press tooltips still announce the full action name. A debounced `resize` listener re-renders the topbar on orientation change. **(2) Spot card flips to horizontal layout on mobile:** CSS class hooks `trip-spot-card`, `trip-spot-cover`, `trip-spot-cover-empty`, `trip-spot-body`, `trip-spot-actions` added to the HTML strings built by `_tripRenderSpotCard()`. A `@media (max-width:639px)` block in `home_page_trip.html` switches `.trip-spot-card` to `flex-direction:row`, gives `.trip-spot-cover` a fixed 96 × auto square thumbnail, shrinks the emoji placeholder font, tightens `.trip-spot-body` padding, and gives action buttons `min-height:1.75rem` for reliable tap targets. Desktop layout (`≥ 640px`) is entirely unchanged. |
