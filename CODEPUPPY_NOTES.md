...
> Last recorded session: **2026-05-13** (Grid mobile layout fix — full-width preset columns on mobile).
...
| Date | What happened |
|---|---|
| 2026-05-13 | **Trip Research view — mobile UX follow-up (commit `0eccaec`).** Three more mobile improvements. **(1) Type-filter pills hidden on mobile:** `_tripRenderFilterBar()` in `home-page-trip-filters.js` now wraps the separator + pills in `<div class="trip-type-pills-wrap">`. A `@media (max-width:639px)` rule in `home_page_trip.html` hides that div — All / Restaurant / Hotel / … pills disappear on small screens (they’re still accessible via the funnel Sort/Group/Filter panel). **(2) Spot cards taller on mobile:** The previous horizontal-card layout is replaced with a larger vertical card — `.trip-spot-cover` grows to `12rem` (was `h-40` = 10rem) and `.trip-spot-cover-empty` grows to `9rem` with a `3.5rem` emoji, so the cover image is the dominant visual element on a full-width single-column card. **(3) Tapping the card opens the Edit Spot modal:** `onclick="tripOpenEditSpot(s.id)"` added to the outer `.trip-spot-card` div. All inner interactive elements (stars, map link, pin button, edit button, delete button) already had `event.stopPropagation()`. The day-section `<select>` and `＋ Day` button were missing it — both now have `event.stopPropagation()` so tapping those controls doesn’t inadvertently open the edit modal. HTML5 drag-and-drop natively suppresses the click event after a real drag so no extra guard is needed on desktop. |
