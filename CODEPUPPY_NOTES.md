...
| `static/js/home-page-uploads.js` | Uploads page JS module. State: `_uplGrouped` (grouped-by-MIME display toggle). Detail panel (image/video/audio/embed/text preview), filter tabs, tag rendering, `_uplJsStr()` / `_uplEsc()` escaping helpers. Tags CRUD delegates to companion file. |
