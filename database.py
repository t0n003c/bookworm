...
# The connection factory + DB paths now live in core.db (the DB-session layer).
# Re-exported here so existing `from database import get_db / DB_PATH / _DATA_DIR`
# imports keep working unchanged. This module keeps the schema + init_db()
# migrations. See ARCHITECTURE.md (Phase 2).
...
# get_db() lives in core.db now (imported + re-exported at the top of this file).
