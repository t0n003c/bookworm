"""db.migrations — init_db(): create tables, run additive/idempotent migrations,
seed defaults.

ALL schema changes live here. Every migration must be additive + idempotent
(safe to run repeatedly on a live DB). Consumes the schema/seed constants from
db.schema and the DB path from core.db. Re-exported as `database.init_db` for
backward compatibility. See ARCHITECTURE.md (Phase 2b).
"""
import json
import os
